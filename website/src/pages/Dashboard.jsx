...
const { equipmentData, conditionOptions } = data;

const Dashboard = () => {
  const { equipment, updateQuantity, toggleInspected } = useEquipmentStore();
  const { infrastructure, updateCondition } = useInfrastructureStore();

  // Map equipment to quantities for display
  const quantities = {};
  const inspected = {};
  equipment.forEach(item => {
    quantities[item.id] = item.quantity;
    inspected[item.id] = item.isInspected;
  });
  ...
  const handleConditionChange = (pointId, newCondition) => {
    updateCondition(pointId, newCondition);
  };
